...
class HtmlIndexHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/revert":
            ...
            back_index = payload.get("index", 0)
            if not file_path or not isinstance(back_index, int):
                self.send_error(400)
                return
            ...
            if back_index < 0 or back_index >= len(snapshot_dirs):
                self.send_error(404)
                return
            snapshot_dir = snapshot_dirs[back_index]
