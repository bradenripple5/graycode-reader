...
class HtmlIndexHandler(SimpleHTTPRequestHandler):
    def do_GET(self) -> None:
        if self.path == "/events":
            ...
            self.end_headers()
            with _client_lock:
                _clients[self.wfile] = self.client_address
                print(f"sse connected: {self.client_address}")
            try:
                while True:
                    time.sleep(3600)
            except Exception:
            ...
            if snapshot_dir is None:
                snapshot_dirs = sorted(
                    [
                        ...
                        if p.is_dir() and p.name != "latest.json"
                    ],
                    key=lambda p: p.name,
                    reverse=True,
                )
...
def broadcast_reload(source: str | None = None) -> None:
    ...
    if source:
        targets = ", ".join(str(_clients[c]) for c in _clients)
        print(f"reload broadcast: {source} -> [{targets}]")
